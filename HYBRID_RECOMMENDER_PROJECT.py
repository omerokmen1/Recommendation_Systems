...
# movie.csv > movieId, film adı ve filmin tür bilgilerini içeren veri seti
# movieId: Eşsiz film numarası.
# title: Film adı
# genres: Tür

# rating.csv > UserID, film adı, filme verilen oy ve zaman bilgisini içeren veri seti
...
df = movie.merge(rating, how="left", on="movieId")  # movieId ortak kolonuna göre birleştirme işlemini yapıyoruz.
df.head()
df.shape
...
comment_counts = pd.DataFrame(df["title"].value_counts())   # Kalabalıkların bilgeliği - Genellenebilirlik
comment_counts
...
# Adım 5: Yukarıda yapılan tüm işlemleri fonksiyonlaştırınız.
...
random_user_df = user_movie_df[user_movie_df.index == random_user]  # Bu şekilde sadece kullanıcımızın izlediği filmleri getiriyoruz.
random_user_df.head()
random_user_df.shape

# Adım 3: Seçilen kullanıcının oy kullandığı filmleri movies_watched adında bir listeye atayınız.
movies_watched = random_user_df.columns[random_user_df.notna().any()].to_list()     # Filmleri isimleri sütunlarda olduğundan dolayı çağırma işlemini "columns" üzerinen yapıyoruz.
movies_watched      # NaN olmayanları listeye atıyoruz.
...
# Adım 1: Seçilen kullanıcının izlediği fimlere ait sütunları user_movie_df'ten seçiniz ve movies_watched_df adında yeni bir dataframe oluşturunuz.
movies_watched_df = user_movie_df[movies_watched]   # Tüm filmler içerisinden kullanıcımızın izlediği filmleri seçiyoruz.
movies_watched_df.head()
movies_watched_df.shape
user_movie_df.shape

# Adım 2: Her bir kullanıcının seçili user'in izlediği filmlerin kaçını izlediği bilgisini taşıyan user_movie_count adında yeni bir dataframe oluşturunuz.
user_movie_count = movies_watched_df.T.notnull().sum()      # Boş olmayanların toplamını getiriyoruz. Çünkü boş olanlar kullanıcımızın izlemediği filmleri gösteriyor.
user_movie_count = user_movie_count.reset_index()
user_movie_count.columns = ["userId", "movie_count"]    # Sütunların isimlerini değiştiriyoruz.
...
corr_df.head()
corr_df[corr_df["user_id_1"] == random_user]        # user_id_1'i kullanıcımız olarak seçiyoruz.
...
top_users_ratings = top_users.merge(rating[["userId", "movieId", "rating"]], how='inner')   # Bu şekilde ortak kesişimler geliyor.
top_users_ratings = top_users_ratings[top_users_ratings["userId"] != random_user]   # random_user verisini düşürüyoruz. Çünkü kullanıcımızın Id'sini veriyor.
...
top_users_ratings['weighted_rating'] = top_users_ratings['corr'] * top_users_ratings['rating']  # Ağırlıklandırılmış bir skor oluşturuyoruz.
top_users_ratings.head()

# Adım 2: Film id’sine her bir filme ait tüm kullanıcıların weighted rating’lerinin ortalama değerini içeren recommendation_df adında yeni bir dataframe oluşturunuz.
recommendation_df = top_users_ratings.groupby('movieId').agg({"weighted_rating": "mean"})
recommendation_df = recommendation_df.reset_index()     # Çoklayan movie'lerin ortalamasını alarak movie'leri tekilleştirdik.
...
movies_from_item_based[1:6].index   # 1'den 6'ya kadar sıraladık. 0'da filmin kendisi var. Onu dışarda bıraktık.
